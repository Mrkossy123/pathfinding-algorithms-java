...
public class Ucs{
    ...
    public static void uniformCostSearch(Node start, Node goal1,Node goal2){
        ...
        PriorityQueue<Node> queue = new PriorityQueue<Node>(2000, 
            new Comparator<Node>()//we override the queue to return the node with the smallest pathValue
            {
                ...
                public int compare(Node i, Node j){
                    if(i.pathValue > j.pathValue){
                        return 1;
                    }
                    else if (i.pathValue < j.pathValue){
                        return -1;
                    }
                    else{
                        return 0;
                ...
                {      
                    ...
                    if(child.value!=-1)//only walls have values of -1. This prevents the traversal of walls
                    {
                        ...
                        else if((queue.contains(child)))
                        {
                            ...
                            if(child.pathValue>diagPathValue&& neighborType==current.diagNeighbors) 
                            {
                            ...
                            else if(child.pathValue>linearPathValue && neighborType==current.linearNeighbors)
                            {
                                child.parent=current;
